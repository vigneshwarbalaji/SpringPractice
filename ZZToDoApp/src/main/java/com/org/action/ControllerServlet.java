...
//				map.put("userdetail",userdetail);
	...
	public String AddTask() throws IOException
	{
		...
		AddTaskModel add = new AddTaskModel();
		
		add.setId(null);
		add.setEmail(email);
		add.setTask(task);
		add.setTaskDescription(description);
		
		boolean result = dao.createTask(add);
		
		if(result == true)
		{
			map.put("value","true");
			map.put("taskList",add);
		}
	...
	@Path("/ViewTask")
	@Produces("application/json")
	public String ViewTask() throws IOException
	{
		HttpSession session = request.getSession();
		
		String email = session.getAttribute("email").toString();
		
		List<AddTaskModel>task = dao.viewTask(email);
...
//		InputStream st=servletContext.getResourceAsStream("/");
//		BufferedReader buffReader = new BufferedReader(new InputStreamReader(st));
//
//		String string = new String();
//		String modifiedHtml = "";
